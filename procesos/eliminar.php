...
<body>
	...
	$registros=mysqli_query($conexion,"select id from usuarios
		where id='$_REQUEST[id]'") or

	die("Problemas en el select:".mysqli_error($conexion)); if ($reg=mysqli_fetch_array($registros)) 

		mysqli_query($conexion, "delete from usuarios where id='$_REQUEST[id]'") or die("Problemas en el select:".mysqli_error($conexion));
